...
from utils.dynamic_loss import total_loss, proximal_l1, landmark_ranking_loss
...
def mean_std_over_folds(fold_results):
    """fold_results: list of dict {(s,d): {'cindex':.., 'cindex_harrell':.., 'brier':..}}"""
    if not fold_results:
        return {}
    ...
    for k in keys:
        ...
        chs = [f[k].get('cindex_harrell', float('nan')) for f in fold_results
               if not np.isnan(f[k].get('cindex_harrell', float('nan')))]
        ...
        agg[k] = {
            ...
            'cindex_harrell_mean': float(np.mean(chs)) if chs else float('nan'),
            'cindex_harrell_std': float(np.std(chs)) if chs else float('nan'),
...
def train_one_fold(fold_idx, train_idx, val_idx, processed, args, device):
    ...
    model = ReKDynSurv(
        ...
        train_landmark_mask_p=args.train_landmark_mask_p,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[fold {fold_idx}] model params = {n_params}")

    # initialise hazard head bias from empirical period event rate so the model
    # starts near a calibrated baseline rather than the hardcoded -2.2.
    event_rate = float(processed['E'][train_idx].sum()) / max(1, len(train_idx) * K)
    event_rate = float(np.clip(event_rate, 1e-3, 0.5))
    bias_init = math.log(event_rate / (1.0 - event_rate))
    with torch.no_grad():
        model.hazard_head.bias.fill_(bias_init)
    print(f"[fold {fold_idx}] hazard bias init = {bias_init:.3f} (event rate = {event_rate:.4f})")
    ...
    for epoch in range(args.epochs):
        model.train()
        train_stats = {'total': 0.0, 'haz_nll': 0.0, 'rank': 0.0, 'aux': 0.0,
                       'ent_reg': 0.0, 'lm_rank': 0.0}
        n_batches = 0
        for batch in train_loader:
            ...
            # Landmark-aware ranking: do a masked forward at a sampled landmark
            # and apply ranking restricted to the (s, Δ) evaluation cells.
            lm_rank_total = torch.zeros((), device=device)
            if args.alpha_landmark_rank > 0 and len(args.landmarks) > 0:
                s_choice = int(np.random.choice(args.landmarks))
                if s_choice < K:
                    out_lm = model(batch['env_periods'], batch['static'],
                                   env_mask_from=s_choice)
                    n_terms = 0
                    for d in args.horizons:
                        if s_choice + d > K:
                            continue
                        lm_rank_total = lm_rank_total + landmark_ranking_loss(
                            out_lm['cif'], batch['T'], batch['E'],
                            landmark=s_choice, horizon=d,
                            sigma=args.sigma_rank,
                        )
                        n_terms += 1
                    if n_terms > 0:
                        lm_rank_total = lm_rank_total / n_terms

            total = loss_dict['total'] + args.alpha_landmark_rank * lm_rank_total
            ...
            train_stats['lm_rank'] += float(lm_rank_total.detach().item())
            n_batches += 1
        ...
        track_val = val_results.get(track_cell, {'cindex_harrell': float('nan')}).get(
            'cindex_harrell', float('nan'))
        if np.isnan(track_val):
            # fallback: average over all cells using Harrell C
            cs = [v.get('cindex_harrell', float('nan'))
                  for v in val_results.values()
                  if not np.isnan(v.get('cindex_harrell', float('nan')))]
        ...
        if epoch % 5 == 0 or epoch == args.epochs - 1:
            ...
            print(
                ...
                f"lm_rank {train_stats['lm_rank']:.3f} "
...
def main():
    ...
    parser.add_argument('--max_folds', type=int, default=0,
                        help='If >0, stop after running this many folds (useful for smoke tests).')
    ...
    parser.add_argument('--tau_l1', type=float, default=0.0)

    # loss weights
    parser.add_argument('--alpha_haz', type=float, default=0.3)
    parser.add_argument('--alpha_rank', type=float, default=2.0)
    parser.add_argument('--alpha_aux', type=float, default=0.3)
    parser.add_argument('--alpha_ent', type=float, default=0.0)
    parser.add_argument('--sigma_rank', type=float, default=0.5)
    parser.add_argument('--event_weight', type=float, default=5.0)
    parser.add_argument('--alpha_landmark_rank', type=float, default=1.0)
    parser.add_argument('--train_landmark_mask_p', type=float, default=1.0)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(
        ...
        if args.max_folds > 0 and fold_idx >= args.max_folds:
            print(f"\nReached --max_folds={args.max_folds}, stopping early.")
            break
    ...
    print("\n===== SUMMARY (mean ± std over {} folds) =====".format(len(fold_results)))
    print(f"{'(s, Δ)':<10} {'C^td_IPCW':>20} {'C_Harrell':>20} {'BS^td':>20}")
    for (s, d), v in sorted(agg.items()):
        print(f"{str((s,d)):<10} "
              f"{v['cindex_mean']:.4f} ± {v['cindex_std']:.4f}    "
              f"{v['cindex_harrell_mean']:.4f} ± {v['cindex_harrell_std']:.4f}    "
