...
def time_dependent_cindex_harrell(cif: np.ndarray, T: np.ndarray, E: np.ndarray,
                                  landmark: int, horizon: int) -> float:
    """Harrell's C-index restricted to the risk set at landmark s, evaluated
    on F(s+Δ). More stable than IPCW when sample size or event count is small,
    so we use it for early-stop tracking; IPCW remains the reported metric.
    """
    K = cif.shape[1]
    ...
    if E_eval.sum() < 1:
        return float('nan')
    risk_score = cif[risk_mask, eval_idx]
    try:
        c, *_ = concordance_index_censored(E_eval, T_eval, risk_score)
        return float(c)
    except Exception:
        return float('nan')
...
def evaluate_all_landmarks(model, val_dataset, T_train: np.ndarray, E_train: np.ndarray,
                           ...
                           batch_size: int = 32,
                           device: torch.device = torch.device('cpu')):
    """Compute C^td (IPCW + Harrell) and BS^td for every (s, Δ) cell.

    Returns dict {(s, Δ): {'cindex': ..., 'cindex_harrell': ..., 'brier': ...}}.
    """
    results = {}
    K = val_dataset.K
    for s in landmarks:
        ...
        for d in horizons:
            ...
            ch = time_dependent_cindex_harrell(cif, T_val, E_val, s, d)
            b = time_dependent_brier(cif, T_val, E_val, s, d, T_train, E_train)
            results[(s, d)] = {'cindex': c, 'cindex_harrell': ch, 'brier': b}
    return results
